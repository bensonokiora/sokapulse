...
import { fetchWeekendFixtures } from '@/utils/api'; // Import the paginated fetch function
import LoadingAnimation from '@/components/LoadingAnimation';
import { getPrediction, getHighestProbability, validatePrediction } from '@/utils/predictionUtils';
import { formatMatchTime } from '@/utils/dateUtils';
import '../styles/custom.css'; // Ensure correct path

export default function WeekendFixturesDisplay({ 
  initialFixtures,
  initialDate, // This is the reference date for the API (e.g., the Saturday of the weekend)
  initialHasMore,
  initialNextCursor,
  initialError = null,
}) {
  const [fixtures, setFixtures] = useState(initialFixtures || []);
  // The date for weekend fixtures is fixed based on what the server determined the weekend to be.
  // It does not change on the client-side for this component.
  ...
  // Removed: allFixtures, visibleCount, observer, lastFixtureElementRef
  ...
  const loadMoreFixtures = async () => {
    ...
    try {
      ...
      console.log(`(Client-Weekend) Loading more. Date: ${date}, Cursor: ${nextCursor}`);
      const response = await fetchWeekendFixtures(date, 20, nextCursor);
      ...
      if (response.status === true && Array.isArray(response.data)) {
        ...
        console.error('(Client-Weekend) API error on load more:', response.message);
        setError(response.message || 'Failed to load more fixtures');
        setHasMore(false);
        setNextCursor(null);
      }
    } catch (err) {
      console.error('(Client-Weekend) Error loading more fixtures:', err);
  ...
  // Use weekend-specific links
  const renderPredictionNav = () => (
    <div className="match-details-navigation">
        <div className="match-details-nav-item">
          <Link href="/weekend-football-predictions" className="match-details-nav-link active ">
            <span>1x2</span>
          </Link>
        </div>
        <div className="match-details-nav-item">
  ...
  const getFixtureDetails = (fixture) => {
    const scores = JSON.parse(fixture.scores || '{ "halftime": { "home": null, "away": null }, "fulltime": { "home": null, "away": null } }');
      ...
      <div>
        <div>
          <div className="space-y-4">
            {fixtures.length === 0 && !error && !isLoadingMore && (
              <div className="text-center p-4">No weekend fixtures available for the current period.</div>
            )}
            ...
            {hasMore && !isLoadingMore && (
              <div className="load-more text-center p-4">
                <button 
                  ...
                  disabled={isLoadingMore} // Only disable if actively loading more
                >
                  Load More Weekend Matches
                </button>
              </div>
            )}
            {!hasMore && fixtures.length > 0 && !isLoadingMore && (
                <div className="text-center p-4 text-gray-500">All weekend matches for this period are loaded.</div>
            )}
          </div>
        </div>
      </div>
