...
import { fetchUpcomingFixtures } from '@/utils/api'; // Import the master fetch function
...
import '@/styles/custom.css'; // Ensure path is correct if needed

export default function UpcomingDoubleChancePredictionsClient({ 
  initialFixtures = [], 
  initialSelectedDate,
  initialHasMore,
  initialNextCursor,
  initialError = null, // Server-side error for initial load
}) {
  const [isLoading, setIsLoading] = useState(false); // For date changes / initial client-side fetch
  const [isLoadingMore, setIsLoadingMore] = useState(false);
  const [fixtures, setFixtures] = useState(initialFixtures);
  const [selectedDate, setSelectedDate] = useState(initialSelectedDate);
  const [nextCursor, setNextCursor] = useState(initialNextCursor);
  const [hasMore, setHasMore] = useState(initialHasMore);
  const [error, setError] = useState(initialError); // Combines initial server error and subsequent client errors
  const [favoriteStates, setFavoriteStates] = useState({});
  const [isMobile, setIsMobile] = useState(false);
  // const [allFixtures, setAllFixtures] = useState(initialFixtures); // Removed
  // const [visibleCount, setVisibleCount] = useState(20); // Removed
  ...
  const fetchDataForDate = async (dateToFetch) => {
    ...
    setHasMore(true);   // Optimistic

    try {
      const formattedDate = formatDateForApi(dateToFetch);
      console.log('(Client-DC) Fetching initial fixtures for date:', formattedDate);
      ...
      if (response.status === true && Array.isArray(response.data)) {
        ...
        console.error('(Client-DC) API error on date change:', response.message);
      ...
      console.error('(Client-DC) Fetch error on date change:', err);
  ...
  const handleDateChange = (e) => {
    ...
    window.history.pushState({}, '', `/upcoming-football-predictions/double-chance-predictions?date=${newDate}`);
  ...
  const loadMoreFixtures = async () => {
    ...
    try {
      ...
      console.log(`(Client-DC) Loading more. Date: ${formattedDate}, Cursor: ${nextCursor}`);
      ...
      if (response.status === true && Array.isArray(response.data)) {
        ...
        console.error('(Client-DC) API error on load more:', response.message);
        setError(response.message || 'Failed to load more fixtures');
        setHasMore(false);
        setNextCursor(null);
      }
    } catch (err) {
      console.error('(Client-DC) Error loading more fixtures:', err);
  ...
  useEffect(() => {
    // This effect synchronizes state if initial props change, e.g., due to SSR re-render with different query params
    // though with client-side navigation for date, this might be less critical after initial load.
    setFixtures(initialFixtures || []);
    setSelectedDate(initialSelectedDate || formatDateForApi(new Date())); // Default to today if prop undefined
  ...
  useEffect(() => {
    ...
    // Listener for storage changes (e.g. other tabs)
  ...
  const renderPredictionNav = () => (
    <div className="match-details-navigation">
      ...
      <div className="match-details-nav-item">
        <Link href="/upcoming-football-predictions/double-chance-predictions" className="match-details-nav-link active">
  ...
  return (
    <>
      ...
      {isLoading ? (
        ...
        <div className="no-fixtures-message text-center p-4 text-gray-500">No double chance predictions available for {selectedDate}</div>
      ) : (
        <>
