...
export default function TeamDoubleChanceClient({ teamData }) {
  ...
  const getFormattedProbabilities = () => {
    const home = teamData?.matchDetails?.percent_pred_home?.replace('%', '') || '0';
    const draw = teamData?.matchDetails?.percent_pred_draw?.replace('%', '') || '0';
    const away = teamData?.matchDetails?.percent_pred_away?.replace('%', '') || '0';
