...
import oneXBetJackpotContent from '@/data/seo/pages/1xbet-jackpot-predictions';
import OneXBetJackpotDisplay from '@/components/OneXBetJackpotDisplay';
import { Metadata } from 'next';

export async function generateMetadata() {
  return {
    title: oneXBetJackpotContent.title,
    description: oneXBetJackpotContent.description,
    keywords: oneXBetJackpotContent.keywords,
    other: {
      'script[type="application/ld+json"]': JSON.stringify(oneXBetJackpotContent.schema),
    }
  };
}

export default async function OneXBetJackpotPredictionPage() {
  let initialPredictions = [];
  let firstMatchDateISO = null;
  let error = null;

  // Parameters for 1xBet Jackpot (bookmakerId=20, type=15, total=17)
  const bookmakerId = 20;
  const typeId = 15;
  ...
  return (
    <div className="jp-container">
      <Suspense fallback={
        ...
        <OneXBetJackpotDisplay 
          initialPredictions={initialPredictions} 
          firstMatchDateISO={firstMatchDateISO} 
          serverError={error}
          seoContent={oneXBetJackpotContent}
