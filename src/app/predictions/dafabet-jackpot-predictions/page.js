...
import dafabetJackpotContent from '@/data/seo/pages/dafabet-jackpot-predictions';
import DafabetJackpotDisplay from '@/components/DafabetJackpotDisplay';
import { Metadata } from 'next';

export async function generateMetadata() {
  return {
    title: dafabetJackpotContent.title,
    description: dafabetJackpotContent.description,
    keywords: dafabetJackpotContent.keywords,
    other: {
      'script[type="application/ld+json"]': JSON.stringify(dafabetJackpotContent.schema),
    }
  };
}

export default async function DafabetJackpotPredictionPage() {
  let initialPredictions = [];
  let firstMatchDateISO = null;
  let error = null;

  // Parameters for Dafabet Jackpot (bookmakerId=20, type=15, total=17)
  const bookmakerId = 11;
  const typeId = 11;
  const totalMatches = 11;

  try {
    console.log(`Fetching Dafabet Jackpot (ID: ${bookmakerId}) predictions server-side...`);
  ...
  return (
    <div className="jp-container">
      <Suspense fallback={
        ...
        <DafabetJackpotDisplay 
          initialPredictions={initialPredictions} 
          firstMatchDateISO={firstMatchDateISO} 
          serverError={error}
          seoContent={dafabetJackpotContent}
