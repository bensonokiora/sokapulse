...
import mathematicalMegaJackpotContent from '@/data/seo/pages/mathematical-mega-jackpot-prediction';
import SportpesaMegaJackpotDisplay from '@/components/SportpesaMegaJackpotDisplay';
import { Metadata } from 'next';

export async function generateMetadata() {
  return {
    title: mathematicalMegaJackpotContent.title,
    description: mathematicalMegaJackpotContent.description,
    keywords: mathematicalMegaJackpotContent.keywords,
    other: {
      'script[type="application/ld+json"]': JSON.stringify(mathematicalMegaJackpotContent.schema),
    }
  };
}

export default async function MathematicalMegaJackpotPredictionPage() {
  let initialPredictions = [];
  let firstMatchDateISO = null;
  let error = null;

  try {
    console.log("Fetching Mathematical Mega Jackpot predictions server-side...");
    ...
    <div className="jp-container">
      <Suspense fallback={
        ...
        <SportpesaMegaJackpotDisplay 
          ...
          serverError={error}
          seoContent={mathematicalMegaJackpotContent}
        />
      </Suspense>
    </div>
  );
