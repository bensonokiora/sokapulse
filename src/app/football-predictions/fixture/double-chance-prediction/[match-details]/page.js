...
export default function MatchDetails() {
  ...
  const validatePrediction = (prediction, scores, status) => {
    ...
    switch (prediction) {
      ...
      case "12": // Home win or away win (no draw)
        predictionCorrect = homeScore !== awayScore;
        break;
      // Also handle standard 1X2 predictions
      case "1": // Home win
        predictionCorrect = homeScore > awayScore;
        break;
      case "2": // Away win
  ...
  // Add function to get double chance probabilities
  const getDoubleChanceProbabilities = (home, draw, away) => {
    const homeProb = parseInt(home);
    const drawProb = parseInt(draw);
    const awayProb = parseInt(away);
  ...
  // Add function to get highest double chance probability
  const getHighestDoubleChanceProb = (probabilities) => {
    const { homeDrawProb, homeAwayProb, drawAwayProb } = probabilities;
    const max = Math.max(homeDrawProb, homeAwayProb, drawAwayProb);
    return max === homeDrawProb ? '1X' : max === drawAwayProb ? 'X2' : '12';
  };
                ...
                <div className="match-info-column">
                  ...
                  <div className="prediction-badge">
                    ...
                    <span className="number-circle rounded-square" 
                          style={validateDoubleChancePrediction(
                            getDoubleChancePrediction(
                              probabilities.home,
                              probabilities.draw,
                              probabilities.away,
                              matchData?.double_chance_goals
                            ).prediction,
                            matchData?.status_short || 'NS',
                            matchData?.goals_home,
                            matchData?.goals_away,
                            {
          ...
          <div className="responsive-row" style={{ cursor: 'auto' }}>
            ...
            <div className="responsive-cell" title="Odds">
              <div className="odds-container">
                {(JSON.parse(matchData?.double_chance_goals || '[]').slice(0, 3)).map((odd, index) => (
                  <div key={index} style={{ fontWeight: getDoubleChancePrediction(
                    probabilities.home,
                    probabilities.draw,
                    probabilities.away,
                    matchData.double_chance_goals
                  ).prediction === ['1X', 'X2', '12'][index] ? 'bold' : 'normal' }}>
                    {odd.odd || '-'}
                  </div>
                ))}
              </div>
