...
export default function MatchDetails() {
  ...
  const handleDateChange = (date) => {
    ...
    // Add any additional logic needed when date changes
  };
  ...
  // Fix the refreshMatchData function to handle the error properly
      ...
      <div className="error-container">
        <div className="error-card">
          ...
          <p className="error-message">
            We couldn't find the match you're looking for. This could be because:
          </p>
          <ul className="error-reasons">
            <li>The match has been rescheduled or canceled</li>
            <li>Our data provider might be experiencing issues</li>
          </ul>
                ...
                <div className="match-info-column">
                  ...
                  <div className="prediction-badge">
                    ...
                    <span className="number-circle rounded-square" 
                          style={validatePrediction(
                            getPrediction(probabilities.home, probabilities.draw, probabilities.away),
          ...
          <div className="responsive-row" style={{ cursor: 'auto' }}>
              ...
              {probabilities && (
                <div className="probability-container" style={{ 
                  ...
                  <div className="probability-item" style={{ 
                    fontWeight: getHighestProbability(probabilities.home, probabilities.draw, probabilities.away) === 'home' ? 'bold' : 'normal',
                    border: '1px solid #dee2e6',
                    borderRadius: '3px',
                    padding: '2px 4px',
                    backgroundColor: getHighestProbability(probabilities.home, probabilities.draw, probabilities.away) === 'home' ? '#e3f2fd' : 'transparent',
                    textAlign: 'center',
                    fontSize: '0.8rem',
                    width: '38px'
                  }}>{probabilities.home}%</div>
                  <div className="probability-item" style={{ 
                    fontWeight: getHighestProbability(probabilities.home, probabilities.draw, probabilities.away) === 'draw' ? 'bold' : 'normal',
                    border: '1px solid #dee2e6',
                    borderRadius: '3px',
                    padding: '2px 4px',
                    backgroundColor: getHighestProbability(probabilities.home, probabilities.draw, probabilities.away) === 'draw' ? '#e3f2fd' : 'transparent',
                    textAlign: 'center',
                    fontSize: '0.8rem',
                    width: '38px'
                  }}>{probabilities.draw}%</div>
                  <div className="probability-item" style={{ 
                    fontWeight: getHighestProbability(probabilities.home, probabilities.draw, probabilities.away) === 'away' ? 'bold' : 'normal',
                    border: '1px solid #dee2e6',
                    borderRadius: '3px',
                    padding: '2px 4px',
                    backgroundColor: getHighestProbability(probabilities.home, probabilities.draw, probabilities.away) === 'away' ? '#e3f2fd' : 'transparent',
                    textAlign: 'center',
                    fontSize: '0.8rem',
                    width: '38px'
                  }}>{probabilities.away}%</div>
                </div>
              )}
            </div>
            ...
            <div className="responsive-cell hide-on-mobile" title="Average Goals">
              {(() => {
                    ...
                    const homeFor = parseInt(matchData?.teams_perfomance_home_for) || 0;
                    const homeAganist = parseInt(matchData?.teams_perfomance_home_aganist) || 0;
                    const awayFor = parseInt(matchData?.teams_perfomance_away_for) || 0;
                    const awayAganist = parseInt(matchData?.teams_perfomance_away_aganist) || 0;
                    const homePlayed = parseInt(matchData?.teams_games_played_home) || 0;
                    const awayPlayed = parseInt(matchData?.teams_games_played_away) || 0;
                    const totalPlayed = homePlayed + awayPlayed;
                    
                    const avgGoalsValue = matchData?.avg_performance ?
                      parseFloat(matchData.avg_performance).toFixed(2) :
                      totalPlayed > 0 ? 
                        ((homeFor + homeAganist + awayFor + awayAganist) / totalPlayed).toFixed(2) : 
                        'N/A';
                    return avgGoalsValue;
                })()}
            </div>
            <div className="responsive-cell" title="Odds">
              <div className="odds-container">
                <div style={{ fontWeight: getPrediction(probabilities.home, probabilities.draw, probabilities.away) === '1' ? 'bold' : 'normal' }}>
                  {matchData?.bets_home || '-'}
                </div>
                <div style={{ fontWeight: getPrediction(probabilities.home, probabilities.draw, probabilities.away) === 'X' ? 'bold' : 'normal' }}>
                  {matchData?.bets_draw || '-'}
                </div>
                <div style={{ fontWeight: getPrediction(probabilities.home, probabilities.draw, probabilities.away) === '2' ? 'bold' : 'normal' }}>
                  {matchData?.bets_away || '-'}
                </div>
              </div>
            </div>
