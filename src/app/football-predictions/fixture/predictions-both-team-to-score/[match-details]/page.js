...
import { getUnderOverPrediction, calculateProbabilities, calculateConfidenceLevel, getBothTeamToScorePrediction, calculateBttsProbabilities, validateBttsPrediction } from '@/utils/predictionUtils';
...
export default function MatchDetails() {
  ...
  // Add BTTS helper functions
  const getBttsProbabilities = () => {
    if (!matchData) return [50, 50];
    return calculateBttsProbabilities(matchData);
  };

  // Get BTTS prediction data
  const getBttsPredictionData = () => {
    if (!matchData) return {
      prediction: 'No',
      odds: 2.0,
      noProb: 50,
    ...
    return getBothTeamToScorePrediction(matchData);
  };
  ...
  // Update prediction validation function for BTTS (Both Teams to Score)
  const validatePrediction = (prediction, scores, status) => {
    ...
    if (prediction === "Yes") {
      ...
      // Handle standard 1X2 predictions as fallback
      switch (prediction) {
        case "1":
  ...
  // Get BTTS prediction data
  const bttsData = getBttsPredictionData();
  const [noProb, yesProb] = getBttsProbabilities();
    ...
    <div className="min-h-screen bg-gray-100">
          ...
          <div className={`responsive-row match-details-header-row ${isMobile ? 'match-details-header-row-mobile' : ''}`}>
            <div className="table-cell match-details-header-cell">
              <h1 className={`match-details-header-title ${isMobile ? 'match-details-header-title-mobile' : ''}`}>
                {matchData?.home_team_name} vs {matchData?.away_team_name} - Both Teams To Score (BTTS) Prediction
              </h1>
            </div>
          </div>
                ...
                <div className="match-info-column">
                  ...
                  <div className="prediction-badge">
                    ...
                    <span className="number-circle rounded-square" 
                          style={validateBttsPrediction(
                            bttsData.prediction,
                            matchData?.parsedScores,
                            matchData?.status_long
                          )}>
                      BTTS: {bttsData.prediction}
                    </span>
                  </div>
                </div>
              )}
          ...
          <div className="responsive-row" style={{ cursor: 'auto' }}>
            ...
            <div className="responsive-cell" style={{ minWidth: '70px', textAlign: 'center' }}>
              <div>
                <span style={{ fontWeight: bttsData.prediction === 'No' ? 'bold' : 'normal' }}>{noProb}%</span>&nbsp;
                <span style={{ fontWeight: bttsData.prediction === 'Yes' ? 'bold' : 'normal' }}>{yesProb}%</span>
              </div>
            </div>
            <div className="responsive-cell" title="Prediction" style={{ minWidth: '60px', textAlign: 'center' }}>
              <span className="m-1">
                <span className="number-circle rounded-square" style={{
                  ...
                  {bttsData.prediction === 'Yes' ? 'BTTS: YES' : 'BTTS: NO'}
                </span>
              </span>
            </div>
