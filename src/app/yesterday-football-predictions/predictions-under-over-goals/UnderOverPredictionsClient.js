...
export default function UnderOverPredictionsClient({ 
  ...
  perPage = 20,
  initialDate // Date for API calls, passed from server
}) {
  const [isLoading, setIsLoading] = useState(!initialFixtures || initialFixtures.length === 0);
  const [isLoadingMore, setIsLoadingMore] = useState(false);
  const [fixtures, setFixtures] = useState(initialFixtures || []);
  const [allFixtures, setAllFixtures] = useState(initialFixtures || []); // To keep track of all loaded fixtures
  ...
  // Selected date is primarily for the NavigationRow, API calls will use initialDate
  ...
  const loadMoreFixtures = useCallback(async () => {
    ...
    try {
      // Use initialDate (yesterday's date from server) for pagination
  ...
  useEffect(() => {
    ...
    setSelectedDate(initialDate); // Ensure selectedDate for NavRow is set
  ...
  const renderPredictionNav = () => (
    <div className="match-details-navigation">
      ...
      <div className="match-details-nav-item">
        <Link href="/yesterday-football-predictions/predictions-under-over-goals" className="match-details-nav-link active">
  ...
  const renderFixture = (fixture, index) => {
    try {
      ...
      let odd = '0.00'; // Default odd
      let expectedGoalsFromUtil = 2.5; // Default expected goals
      ...
      try {
        ...
        if (Array.isArray(predictionResult) && predictionResult.length >= 3) {
          ...
          expectedGoalsFromUtil = typeof predictionResult[2] === 'number' ? predictionResult[2] : 2.5;
        } else if (predictionResult && typeof predictionResult === 'object') {
            // Fallback if it returns an object (based on some util versions)
            prediction = predictionResult.prediction || 'Under';
            odd = predictionResult.odd || '0.00';
            expectedGoalsFromUtil = typeof predictionResult.expectedGoals === 'number' ? predictionResult.expectedGoals : 2.5;
        }
      } catch (error) {
        console.error('Error in getUnderOverPrediction call:', error, 'Fixture data:', fixtureForPrediction);
      }
      
      // Calculate expected goals for display
      // Prioritize fixture.avg_performance if valid, otherwise use value from util, then default
      let displayAvgGoals;
      if (fixture.avg_performance && !isNaN(parseFloat(fixture.avg_performance))) {
        displayAvgGoals = parseFloat(fixture.avg_performance).toFixed(2);
      } else {
        displayAvgGoals = expectedGoalsFromUtil.toFixed(2);
      }
      ...
      let overProb = 50;
      try {
        // Pass the preprocessed fixture to calculateProbabilities as well, in case it also relies on goals_over_under
        ...
        console.error('Error in calculateProbabilities call:', error, 'Fixture data:', fixtureForPrediction);
      }
      
      const { confidenceText, confidenceColor } = calculateConfidenceLevel(underProb, overProb);
      
      const homeGoals = fixture.goals_home !== null && typeof fixture.goals_home !== 'undefined' ? parseInt(fixture.goals_home, 10) : null;
      const awayGoals = fixture.goals_away !== null && typeof fixture.goals_away !== 'undefined' ? parseInt(fixture.goals_away, 10) : null;
      
      const scoresForValidation = { 
        ...scores, // Contains parsed halftime/fulltime/extratime
        fulltime: { // Override fulltime with direct goals_home/goals_away for validation
          home: homeGoals, 
          away: awayGoals 
        }
      };
      ...
      return (
        <div key={fixture.fixture_id || index} className="responsive-row" style={{ cursor: 'auto' }}>
          ...
          <div className="responsive-cell hide-on-mobile" title="Average Goals">
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div>{displayAvgGoals}</div>
              <div style={{ fontSize: '10px', color: '#666' }}>
                {
                  parseFloat(displayAvgGoals) > 3.0 ? 'High-scoring' : 
                  parseFloat(displayAvgGoals) > 2.5 ? 'Above avg' : 
                  parseFloat(displayAvgGoals) > 2.0 ? 'Average' : 'Low-scoring'
                }
              </div>
            </div>
          </div>
          <div className="responsive-cell" title="Odds">
            <div className="odds-container">
              <div>{typeof odd === 'number' ? odd.toFixed(2) : odd}</div>
            </div>
          </div>
  ...
  return (
    <div className="row" style={{ marginLeft: '0px', height: 'auto !important' }}>
      <div className="col-lg-12" style={{ height: 'auto !important' }}>
        <NavigationRow selectedDate={selectedDate} />
        ...
        {isLoading ? (
          ...
          <>
            ...
            {!hasMore && !loadMoreError && allFixtures.length > 0 && !isLoadingMore && (
