...
export default function BothTeamsToScorePredictionsClient({ 
  ...
  const formatDate = (date) => {
    ...
    if (isNaN(d.getTime())) {
      const tomorrowFallback = new Date();
      tomorrowFallback.setDate(tomorrowFallback.getDate() + 1);
      const year_fb = tomorrowFallback.getFullYear();
      const month_fb = String(tomorrowFallback.getMonth() + 1).padStart(2, '0');
      const day_fb = String(tomorrowFallback.getDate()).padStart(2, '0');
      return `${year_fb}-${month_fb}-${day_fb}`;
    }
  ...
  const renderPredictionNav = () => (
    <div className="match-details-navigation">
      ...
      <div className="match-details-nav-item">
        <Link href="/tomorrow-football-predictions/predictions-both-team-to-score" className="match-details-nav-link active">
          <span>BTTS</span>
        </Link>
      </div>
    </div>
