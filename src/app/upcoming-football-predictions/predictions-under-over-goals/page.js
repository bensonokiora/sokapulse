...
// NavigationRow and prediction nav will be handled by the client component
...
export default async function UnderOverPredictionsPage({ searchParams }) { // Renamed function and added searchParams
  ...
  try {
    console.log('SSR Fetching initial under/over upcoming fixtures for date:', initialSelectedDate);
    ...
    if (response.status === true && Array.isArray(response.data)) {
      ...
      console.log("SSR Under/Over Fetch successful:", { 
          initialBatchSize: initialFixtures.length,
          hasMore: initialHasMore,
          nextCursor: initialNextCursor
      });
    } else {
      console.error('SSR API error (under/over):', response.message);
      initialError = response.message || 'Failed to load fixtures on server';
    }
  } catch (err) {
    console.error('SSR Fetch error (under/over):', err);
      ...
      <Suspense fallback={<div className="flex justify-center items-center w-full h-64"><LoadingAnimation text="Loading page..." /></div>}>
        <div className="row" style={{ marginLeft: '0px', height: 'auto !important' }}>
          <div className="col-lg-12">
            {/* Prediction type title can remain or be moved to client */}
            ...
            {/* SEO Content */}
            <SeoContent pageType="upcoming-under-over-goals" siteName="SokaPulse" />
          </div>
        </div>
      </Suspense>
    </div>
